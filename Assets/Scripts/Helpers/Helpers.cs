...
public static class Helpers
{
    ...
    {
        TextMeshProUGUI currentDebugDisplay = GameObject.Instantiate(debugDisplay);
    ...
    {
        ParticleSystem ps = GameObject.Instantiate(prefabPS, t.position, Quaternion.identity);
        ps.Play();
        GameObject.Destroy(ps.gameObject, ps.main.duration + 1f);
    }


    public static void LerpQuaternion(MonoBehaviour context, Transform objectTransform, Quaternion initialPos, Quaternion finalPos, float duration)
    {
        context.StartCoroutine(LerpQuaternionCoroutine(objectTransform, initialPos, finalPos, duration));
    }

    static IEnumerator LerpQuaternionCoroutine(Transform objectTransform, Quaternion initialPos, Quaternion finalPos, float duration)
    {
